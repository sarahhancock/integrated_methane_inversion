...
def read_geoschem(date, gc_cache, config):
    ...
    if UseGCHP:
        ...
        species = np.einsum("lfyx->fyxl", species)
    else:
...
def get_overlap_area_CSgrid(satellite, filename, sat_ind, CSgridDir, 
                            gridspec_path, GC_shape):
    # create regridding weights first
    regrid_weight_fpath = create_ESMF_regridding_weights(satellite, filename, sat_ind, 
